...
class AddFoundController: UIViewController {
    ...
    private let petNameTextField: CustomTextField = CustomTextField(placeholder: "Pet Name(optional)")
    
    private let breedTextField: CustomTextField = CustomTextField(placeholder: "breed(optional)")
    
    private let areaTextField: CustomTextField = CustomTextField(placeholder: "Where do you find the pet(ZIP code)")
    ...
    func updateForm() {
        ...
        if  area?.isEmpty == false  && description?.isEmpty == false
    ...
    @objc func handleAddLost(){
        ...
        let credentials = FoundCredentials(species: species, petName: petName, breed: breed,
                                              area: area, description: description, phone: phone, email: email,
                                              ownerName: ownerName, petImage: petImage, time: Timestamp())
        FoundService.uploadFounds(withCredentials: credentials) { error in
            if let  error = error {
                print("There is an error uploading 'found' post \(error.localizedDescription)")
                return
            }else{
    ...
    func configureUI() {
        ...
        let stack = UIStackView(arrangedSubviews: [ownerTextField, speciesTextfield,petNameTextField, breedTextField, areaTextField, descriptionTextField, phoneTextField, emailTextField, addButton])
        stack.axis = .vertical
        stack.spacing = 15
...
extension AddFoundController: UIImagePickerControllerDelegate, UINavigationControllerDelegate{
